...
def get_dividend(code):
    ...
    result = soup.select_one('em#_dvr')
    if len(result) == 0: 
        return result
    else: 
        return result.text
...
def get_3year_treasury():
    url = "http://www.index.go.kr/strata/jsp/showStblGams3.jsp?stts_cd=288401&amp;idx_cd=2884&amp;freq=Y&amp;period=1998:2020"
...
def get_dividend_dict(code):
    url = "https://finance.naver.com/item/coinfo.nhn?code="+code
    response = requests.get(url)

    soup = BeautifulSoup(response.text, 'html5lib')
    results = soup.select("span.bar")
    return results
...
if __name__ == "__main__":
    ...
    dividend_yield = get_dividend_yield('005930')
    print(dividend_yield)
    dividend_dict = get_financial_statements("005930") 
    print(dividend_dict)
    ...
    # print(get_min_max_dividend_to_treasury('058470'))